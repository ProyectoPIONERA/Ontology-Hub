...
exports.relationSuggest  = function (object) {
    ...
    for (i = 0; i < object.length; i++){
      ...
      if(part['prefix'])html+='<a href=\'/dataset/vocabs/'+part['prefix']+'\'>('+part['prefix']+')</a>';
      html+='</div>';
    }
    return html;
}
...
exports.displayReviews  = function (comment) {
    ...
    for (i = 0; i < comment.length; i++){
      var part = comment[i];
      if (part.agentId){
        html+='<div>('+this.dateToYMD(part.createdAt)+')</span> <a href="/dataset/agents/'+part.agentId["name"]+'">'+part.agentId.name+'</a>: <span>'+part.body+'</div>';
      }
      else{
        html+='<div>('+this.dateToYMD(part.createdAt)+')</span> <span>User eliminated</span>: <span>'+part.body+'</div>';
      }
    }
    return html;
}
